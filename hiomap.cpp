...
{
    // SIGTERM was masked earlier, raise it again so all handlers can respond
    kill(0, SIGTERM);
    return 0;
}

static SignalResponse hiomap_protocol_reset(struct hiomap* ctx)
{
    if (ctx->bmc_events == BMC_EVENT_PROTOCOL_RESET)
    {
        // Allow the default SIGTERM handler to shut down ipmid
    ...
    // ignore the SIGTERM for now until the host command returns
...
static void register_openpower_hiomap_commands()
{
    ...
    registerSignalHandler(ipmi::prioOemBase, SIGTERM, [ctx](int sig) {
